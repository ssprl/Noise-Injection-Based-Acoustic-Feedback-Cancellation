    ...
    for k=1:length(Frame) % k =sample no. of first frame
        ...
        if (mod(((i-1)*n+k),(C-1)*n_j_i)<=n_d) && (((i-1)*n+k)<=(av_num-1)*n_j_i+n_d)
            ...
            if mod(((i-1)*n+k),n_j_i)>length(f)
                ...
                mic_in(m)=y(k);
                final_noise = [final_noise, noise(m)];
                final_mic_in = [final_mic_in, mic_in(m)];
                m=m+1;
                hh=hh+1;
                soft_idx = soft_idx + 1;
            ...
            if mod(((i-1)*n+k),(C-1)*n_j_i)==n_d
                %                 if (mod(i,2)~=0)
                %                     pp1=noise(1:n-length(f));
                %                     pp2=mic_in(1:n-length(f));
                %                 end
                %
                %
                %                 if (mod(i,2)==0)
                ...
                %                     pp2=mic_in(1:n);
                %                 end
                %                 final_noise = [final_noise,pp1];
                %                 final_mic_in = [final_mic_in,pp2];
                [dbmis_GLD,b,C] = GLD(final_mic_in,final_noise,hh,n_d,f,maxlag,F,C,NFFT,number,k,n_j_i);
                dbmis(:,(floor(((i-1)*n+k)/n_j_i)+1))=dbmis_GLD;
            end
        else
...
for uu=1:av_num
    avg_dbmis_GLD(uu) =mean(dbmis(:,uu));
end

[min_dbmis_GLD,index_min_dbmis_GLD]=min(avg_dbmis_GLD);

final_dbmis_GLD =dbmis(:,index_min_dbmis_GLD) ;

plot( final_dbmis_GLD );
